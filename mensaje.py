...
@app.route('/nosotros')
def nosotros():
    grafico_barras = crear_grafico_barras()  # Generar el gráfico de barras
    grafico_pastel = crear_grafico_pastel()  # Generar el gráfico de pastel
...
def enviar_sms():
    ...
    if response['messages'][0]['status'] == '0':
        return redirect(url_for('index', mensaje='Mensaje enviado con éxito!'))
    else:
        return redirect(url_for('index', mensaje=f"Error: {response['messages'][0]['error-text']}"))
